            ...
            <div class="d-flex align-items-center py-2">
                ...
                <div class="text-success nav-link cat-tab3"
                    ...
                    <div class="text-center" style="margin-left: 300px">
                        <h5 class="ms-1" style="color: #247297;">Sourcing SAS List</h5>
                    </div>
                </div>
                {{-- Add Details End --}}
            </div>
            <div>
                <table class="table sasDT table-bordered table-hover text-center">
                    <thead>
                        <tr class=" text-center">
                            <th width="10%">Sl</th>
                            <th width="10%">Image </th>
                            <th width="40%">Product Name </th>
                            <th width="25%">Sales Price</th>
                        ...
                        @if ($products)
                            @foreach ($products as $key => $item)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($product)
                                            <img class="img-fluid rounded-circle"  src="{{ asset($product->thumbnail) }}" width="25" height="25">
                                        @endif
                                    </td>
                                    <td>{{ $product->name }}</td>
                                    <td class=" text-center">{{ $item->sales_price }}</td>
                                    <td>
                                        <a href="{{ route('sas.edit', [$item->ref_code]) }}" class="text-primary" >
